...
class Log:
    def __init__(self, log_each: int, initial_epoch=-1, epoch_increment=1):
    ...
    def train(self, len_dataset: int) -> None:
        self.epoch += 1 if self.epoch < 0 else self.epoch_increment
        if self.epoch == 0:
            self._print_header()
        else:
            self.flush()
    ...
    def flush(self) -> None:
        format_ = lambda loss: '12.4f' if loss < 1000 else '#12.5g'
        ...
        if self.is_train:
            ...
            print(
                f"\r┃{self.epoch:12d}  |{loss:{format_(loss)}}  |{100*accuracy:10.2f} %  |{self.learning_rate:12.3e}  |{self._time():>12}  ┃",
                #f"\r┃{self.epoch:12d}  ┃{loss:12.4g}  │{100*accuracy:10.2f} %  ┃{self.learning_rate:12.3e}  │{self._time():>12}  ┃",
                end="",
                flush=True,
            )
            ...
            print(f"{loss:{format_(loss)}}  |{100*accuracy:10.2f} %  ┃", flush=True)
            #print(f"{loss:12.4g}  │{100*accuracy:10.2f} %  ┃", flush=True)
    ...
    def _train_step(self, model, loss, accuracy, learning_rate: float) -> None:
        ...
        if self.step % self.log_each == self.log_each - 1:
            ...
            format_ = lambda loss: '12.4f' if loss < 1000 else '#12.5g'
            print(
                f"\r┃{self.epoch:12d}  |{loss:{format_(loss)}}  |{100*accuracy:10.2f} %  |{learning_rate:12.3e}  |{self._time():>12}  {self.loading_bar(progress)}",
                end="",
                flush=True,
            )
